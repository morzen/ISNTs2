...
class Crypto(object):
    ...
    def generateRSAKeypair(self, bits=2048):
        ...
        self.localKeypair = M2Crypto.RSA.gen_key(
            bits, 65537, self.__generateKeypairCallback)
    ...
    def generateDHKey(self):
        self.dh = M2Crypto.DH.set_params(
            decToMpi(self.dhPrime), decToMpi(self.dhGenerator))
        self.dh.gen_key()
    ...
    def setRemotePubKey(self, pubKey):
        if type(pubKey) is str:
            ...
            raise exceptions.CryptoError(
                "Public key is not a string or RSA key object.")
    ...
    def readLocalKeypairFromFile(self, file, passphrase):
        ...
        try:
            self.localKeypair = M2Crypto.RSA.load_key(
                file, self.__passphraseCallback)
        except M2Crypto.RSA.RSAError as rsae:
            raise exceptions.CryptoError(str(rsae))
    ...
    def writeLocalKeypairToFile(self, file, passphrase):
        self.__checkLocalKeypair()
        self._keypairPassphrase = passphrase
        self.localKeypair.save_key(
            file, self.aesMode, self.__passphraseCallback)
